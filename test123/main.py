...
from pct_change_trends import trends as pct_trends
...
for entry in terms:
    normalized.append([["asylum " + entry['language'] + " " + entry['nationality']],
                       pct_trends(gov, 'asylum', entry['language'], entry['asylum'], entry['code'], entry['nationality'])])
    normalized.append([["war " + entry['language'] + " " + entry['nationality']],
                       pct_trends(gov, 'war', entry['language'], entry['war'], entry['code'], entry['nationality'])])
    if 'netherlands' in entry:
        normalized.append([["netherlands " + entry['language'] + " " + entry['nationality']],
                           pct_trends(gov, 'netherlands', entry['language'], entry['netherlands'], entry['code'],
                                  entry['nationality'])])
    if 'taliban' in entry:
        normalized.append([["taliban " + entry['language'] + " " + entry['nationality']],
                           pct_trends(gov, 'taliban', entry['language'], entry['taliban'], entry['code'],
                                  entry['nationality'])])
